...
function ProfileHeader({userData, onSave, isOwnProfile}) {
  ...
  const {data:connectionStatus, refech:refetchCoonectionSatus}=useQuery({
    queryKey:["connectionStatus",userData._id],
    queryFn: async () =>await axiosInstance.get(`/connections/status/${userData._id}`),
  })
  ...
  const renderConnectionButton=()=>{
    const baseClass="text-white py-2 px-4 rounded-full transition duration-300 flex justify-center";
        ...
        return(
          <div className="flex gap-2 justify-center">
            <div className={`${baseClass} bg-green-500 hover:bg-green-600`} >
              <UserCheck side={20} className='mr-2' />
              Connected
            </div>
            <button 
